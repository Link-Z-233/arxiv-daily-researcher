...
以 **ArXiv** 为主来源，完整分页扫描首次提交与最后更新；支持目标分类、公告延迟重扫和扫描收据。额外来源区可启用 PRL、PRA/PRB、Nature、Science、Hugging Face Papers 与声明式期刊来源，并可选接入 OpenAlex、Semantic Scholar 增强信息。
...
候选论文先写入 SQLite，再按优先级处理。论文以来源、稳定标识和版本精确去重；失败阶段留在重试队列，报告写入成功后再提交交付账本、扫描水位线和通知发件箱。
...
旧 HTML 报告可一键写入 SQLite 交付账本，避免未来日报重复处理。开启完整修复后，系统迁移兼容 JSON 与关键词库、补全缺失字段、修补原报告，并按自然周生成遗漏论文补充报告。过去日报按日期范围进入持久队列，逐天执行完整研究流程。
...
| 模式                          | 入口与条件                                      | 结果                                                |
| :---------------------------- | :---------------------------------------------- | :-------------------------------------------------- |
| <code>legacy_import</code>    | 数据管理页“读取旧历史”，等待其他任务空闲        | 默认仅登记旧 HTML 的 arXiv 交付；开启完整修复后迁移 JSON / <code>keywords.db</code> 并执行后续维护 |
...
参考文献 PDF 关键词提取独立受开关控制。关闭后，缓存关键词不会参与评分；已提取关键词使用固定高度滚动区域展示。
...
先查看“数据分析 → LLM 健康”，其中展示真实调用的最近终态、连续失败、成功率、最近成功时间和脱敏错误摘要。
...
默认的“读取旧历史”只解析旧 HTML 中已存在的 arXiv 卡片并建立交付账本，不调用 LLM，也不扫描遗漏。它适合升级后先避免重复推送。

“完整修复旧历史”会按稳定论文身份合并记录，并按报告时间选择最新分析；兼容 JSON 与 <code>data/keywords/keywords.db</code> 会一并迁移。旧关键词库保留逐论文关键词、原始日期和规范词别名，HTML 卡片中的关键词则同时写入论文评分记录。缺失 TL;DR、译文或深度分析由“历史数据补全”基于 SQLite 修复并回写原报告；时间段扫描发现的遗漏按 ISO 自然周进入补充报告队列。每份报告仍受本次处理上限约束，剩余项保留为可重试积压。

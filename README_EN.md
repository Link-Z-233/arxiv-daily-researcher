...
<tr><td><b>v4.0</b></td><td>2026-08-22</td><td>🚀 Major release</td><td>SQLite daily history with exact-version delivery, durable pending-paper queue (max_papers_per_run), full arXiv pagination with terminal scan receipts, declarative extra data sources (core narrowed to arXiv + PRL), Hugging Face Papers source, scoring evaluation feedback, Docker image split and security fixes (WebUI bound to 127.0.0.1 by default, no secrets leaked to cron), and a large reliability hardening pass (fail-closed state, atomic delivery, boundary hardening)</td></tr>

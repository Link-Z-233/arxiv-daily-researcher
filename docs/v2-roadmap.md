...
### P2 — 新的默认资格/排序策略（独立提交，依赖 P1 基线）

- 引入策略版本和兼容模式，先在离线标注集上对比 legacy 与候选策略。
- 将资格门槛、主关键词强命中、参考关键词辅助分与作者排序偏好分离。
- 为 score response、日报 Markdown/HTML、通知 Top-N 和缓存 fingerprint 补齐字段与迁移。
- 只有在标注集上的 recall/precision 目标和迁移样本审查达标后，才切换默认值；保留回滚开关。

**验收：** 作者命中但核心方向为零不得合格；主方向强匹配不能因新增低权重参考词而降低资格；新旧策略可在同一冻结数据集比较。

...
**一个影响体验的配置问题（非代码 bug）**：你的 `primary_keywords` 为空、只有 8 个 reference 关键词，V2 评分策略运行在降级模式（日志已警告"未配置可用 PRIMARY_KEYWORDS，以全部关键词作为核心集合降级"），核心相关性门槛 6.0 几乎不可能达到——这是两次实测及格率 0% 的直接原因。**建议配置 3–5 个真正的核心关键词**，及格率和报告质量会立竿见影。

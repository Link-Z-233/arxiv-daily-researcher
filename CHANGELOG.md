...
2. **持久化待处理队列** — 抓取候选先原子写入 SQLite 队列再处理；`max_papers_per_run = 0` 默认处理全部，正数仅限制本次评分/翻译/分析数量，超出部分留队下次优先，失败/重试论文优先于普通积压。
...
6. **评分评估反馈** — 只读评分诊断与运行诊断工具，评分审计保留非敏感证据。
7. **全文 TLDR 溯源** — 报告中标注 TLDR 是否来自全文解析。
8. **WebUI 运行可观测性** — 安全的运行观测、默认折叠的高级诊断、代理默认关闭（保留配置值）、每日推送面板提供 `0 = 全部` 的处理上限。
9. **LLM Token 用量统计** — 每次运行（含成功/失败/中断）结束时把 per-model token 用量持久化到 SQLite；WebUI 新增"用量统计"页：GitHub 风格的每日用量热力图（近一年）+ 可选时间段（7/30/90/365 天/全部）的输入/输出 token 面积图 + 按模型汇总。数据永久保留。
10. **收藏偏好** — WebUI 新增"收藏偏好"页：对已交付论文一键喜欢/不喜欢/清除标记，标记写入 SQLite（清除只写状态不删行，永久存档），并汇总为兴趣画像（收藏作者 Top、收藏领域 Top、命中的主要关键词）。标记仅用于画像汇总，**不影响 AI 评分**。
11. **WebUI 侧边栏导航** — 面板从顶部 11 个 Tab（每次交互渲染全部控件）改为侧边栏单选导航、按页渲染，页面更轻更快；保存未浏览页面时保留磁盘现值，不会被默认值覆盖。
12. **运行监控实时化** — 运行状态与活跃日志尾部每 5 秒自动刷新（`st.fragment`，可开关）；每日推送页常驻"待处理队列 / 失败待重试 / 按当前上限还需 N 次运行"指标。

...
- **旧历史导入与补充报告**：将 v3.2 JSON 历史和 HTML 日报整理到 SQLite
...
可一键读取 v3.2 JSON 历史和 HTML 报告，合并重复分析、登记缺失数据、扫描遗漏论文并自动生成补充报告。过去日报按日期范围进入持久队列，逐天执行完整研究流程。
...
| 模式                          | 入口与条件                                      | 结果                                                |
| :---------------------------- | :---------------------------------------------- | :-------------------------------------------------- |
| <code>legacy_import</code>    | 数据管理页“读取旧历史”，等待其他任务空闲        | 合并 v3.2 JSON/HTML、登记缺失、回扫遗漏、自动补充  |
...
| 趋势研究                    | 按参数哈希生成的趋势研究锁                             |
| 旧历史导入                  | 独占历史活动闸门，等待日常任务、趋势研究和维护任务空闲 |
...
可在“每日推送 → 状态面板 / 运行日志”查看队列状态和 <code>legacy_import_*.log</code>。同一时间保留一个导入请求即可。
</details>

<details>
<summary><b>4. 旧历史导入如何处理重复分析、缺失数据和遗漏论文？</b></summary>

导入按照稳定论文身份合并记录，并按报告时间选择最新分析。缺报告卡、缺译文、缺深度分析和回扫发现的遗漏论文进入 <code>supplement_backlog</code>。导入完成后，系统自动按本次处理上限运行补充流程；成功项目从积压中出账，剩余项目保留到后续批次。

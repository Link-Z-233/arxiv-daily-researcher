...
1. **Docker 镜像拆分与安全修复** — Worker 与 WebUI 依赖分离（`requirements-core.txt` / `requirements-webui.txt`），Worker 不再安装 Streamlit 全家桶；WebUI 面板默认仅绑定 `127.0.0.1:8501`；cron 不再把容器全部环境变量与密钥复制到 `/etc/environment`。
2. **arXiv 公告延迟重扫**、**陈旧处理阶段失效**、**期刊身份在 arXiv 增强后保留**、**评分输出校验**、**LLM 响应规范化与深度分析校验**、**重试时不丢失可选 Semantic Scholar 增强** 等数十项修复。

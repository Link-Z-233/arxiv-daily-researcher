...
# 标题旁的历史遗留徽标：裸版本号（v1/v2…）与重试标记（↻ 重试）。
# 生成端已不再输出；这里在显示层过滤历史报告里的残留，
# 带说明文字的修订标签（🔁 修订版 …）保留。
...
def _strip_legacy_title_badges(report_html: str) -> str:
    ...
    return _RETRY_LABEL_RE.sub("", report_html)
...
def _build_sandboxed_preview_html(report_html: str) -> str:
    ...
    """
    escaped_report = html.escape(_strip_legacy_title_badges(report_html), quote=True)

...
16. **论文检索页** — 基于数据库元数据的全量历史检索：标题/作者/摘要/TLDR/提取关键词匹配 + 来源、处理日期范围、最低总分、只看收藏过滤，分页浏览；数据永久存档，存量随时间增长也不怕找不到。
17. **数据源健康页** — 每个数据源最近 20 次扫描收据聚合：最近状态与时间、成功率、最近新增候选数、最近一次错误，arXiv 主源置顶展示。

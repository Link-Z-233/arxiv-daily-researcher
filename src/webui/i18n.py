...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "dm_legacy_full_repair_help": {
        "zh": "关闭时仅登记旧 HTML 中已有的 arXiv 论文，避免后续日报重复处理。开启时读取旧 JSON、补全缺失字段、扫描遗漏论文并生成补充报告；旧 keywords.db 只会为同一 HTML 论文补齐缺失关键词。",
        "en": "Off: index only arXiv papers already present in legacy HTML so future daily runs do not repeat them. On: read legacy JSON, repair missing fields, scan omissions, and generate supplements; old keywords.db only fills a missing keyword section for the same HTML paper.",
    },

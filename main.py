...
运行示例:
  ...
  python main.py --mode trend_research --keywords "quantum error correction" "fault tolerant" \\
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="[backfill_run] 单个要补跑的过去日期，格式 YYYY-MM-DD（必须早于今天）",
    )
    parser.add_argument(
        "--keywords",
        nargs="+",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="[trend_research/backfill_run] 起始日期，格式 YYYY-MM-DD",
    )
    parser.add_argument(
        "--date-to",
        type=str,
        default=None,
        help="[trend_research/backfill_run] 结束日期，格式 YYYY-MM-DD",
    )
    parser.add_argument(
        "--sort-order",
        type=str,
...
def _backfill_date_range_from_args(args: argparse.Namespace) -> tuple[date, date]:
    ...
    if target and (date_from or date_to):
        raise ValueError("backfill_run 不能同时使用 --target-date 与 --date-from/--date-to")

    if target:
        start = end = date.fromisoformat(target)
    else:
        if not date_from or not date_to:
            raise ValueError("backfill_run 必须指定 --target-date 或完整的 --date-from/--date-to")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.mode == "trend_research":
        ...
        with run_lock(
            ...
            # 历史导入以独占 gate 排队；趋势任务持共享 gate，确保导入不会
            # 与它同时写入 SQLite 的 token/状态数据。
            with legacy_import_activity_gate(logger=logger):
    ...
    if args.mode == "backfill_run":
        ...
        try:
            date_from, date_to = _backfill_date_range_from_args(args)
        except ValueError as exc:
            logger.error("backfill_run 参数无效: %s", exc)
            return 1

        from modes.backfill_queue import enqueue_and_run_backfill_range

        with run_lock("backfill_run"):
            with daily_workflow_gate(
                logger=logger, wait_note="过去日报补跑等待每日研究/补充运行完成"
            ):
                with legacy_import_activity_gate(
                    logger=logger, wait_note="过去日报补跑等待旧历史导入完成"
                ):
                    return enqueue_and_run_backfill_range(date_from, date_to)
    ...
    with run_lock("daily_research"):
        with daily_workflow_gate(
            logger=logger, wait_note="每日研究等待补充运行/过去日报完成"
        ):
            with legacy_import_activity_gate(
                logger=logger, wait_note="每日研究等待旧历史导入完成"
            ):

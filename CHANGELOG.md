...
11. **WebUI 页面结构重组（顶部 Tab）** — 保持顶部 Tab 点击切换；页面精简为 11 个：收藏与论文检索并入「报告查看」页、用量统计/数据源健康/运行诊断合并为「数据分析」页、网络代理并入「高级设置」页。保存未浏览页面时保留磁盘现值，不会被默认值覆盖。
...
22. **报告版本标识精简** — 论文标题旁不再标注 vN；版本只在详情字段（Version: vN，HTML/Markdown 一致）展示，修订版/重试标签保留。

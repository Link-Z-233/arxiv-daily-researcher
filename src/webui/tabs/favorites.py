...
from webui.pagination import render_paginated_items, render_paginated_table
...
def _render_preference_stats(store: DailyResearchStore) -> None:
    ...
    def render_ranked_table(rows: list[dict], columns: list[str], key: str) -> None:
        """Show every ranking row through the shared 5/10-row pager."""
    ...
    with col_authors:
        ...
        if aggregation["authors"]:
            render_ranked_table(
                aggregation["authors"], ["name", "count"], "favorite_top_authors"
            )
        else:
    ...
    with col_keywords:
        ...
        if ranked:
            render_ranked_table(ranked, ["keyword", "count"], "favorite_keywords")
        else:
